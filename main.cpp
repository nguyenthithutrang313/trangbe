    ...
    while(!quit)
       {
        ...
        /*std::string highest_text="Highest: ";
        text_highest.SetText(highest_text);
        text_highest.LoadText(g_font_text, gScreen);
        text_highest.RenderText(gScreen,1230,20);
        int m_highest = manage_block.GetHighestScore();
        int& m;
        std::string highest_str = std::to_string(m);
        highest.SetText(highest_str);
        highest.LoadText(g_font_text, gScreen);
        highest.RenderText(gScreen,1400,20);*/
        ...
        if(GameOver==true)
        {
            ...
            cout << endl;
            Sleep(500);
            int ret_menu= SDLCommonFunc::ShowMenu(gScreen,g_font_menu," ","Exit","img//gameover1.png");
            if(ret_menu==1)
            {   quit=true;
                continue;
            }
